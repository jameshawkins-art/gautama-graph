...
The Phase 2 blueprint team MUST query `gautama-graph path "<A>" "<B>"` (or `make graphify-path A="<A>" B="<B>"`), `gautama-graph explain "<concept>"` (or `make graphify-explain C="<concept>"`), or `gautama-graph query "<package/interface>"` (or `make graphify-query Q="<package/interface>"`) to construct clean Go interface contracts, AST walker hierarchies, Python IPC bridge signatures, and storage paths with minimal token consumption instead of reading raw files.
