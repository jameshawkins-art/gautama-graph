...
- **AST Production Caller Verification**:
  ...
  - Use `gautama-graph query` (or `make graphify-query`), `gautama-graph path` (or `make graphify-path`), or `go run cmd/graphify-ast-audit/main.go` to confirm verified AST caller edges.
